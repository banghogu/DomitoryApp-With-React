...
const RangePicker = ({ startDate, endDate, onChange }: RangePickerProps) => {
  ...
  const handleDayClick = (dateRange: DateRange | undefined) => {
    ...
    onChange({
      from: from != null ? (format(from, 'yyyy-MM-dd') as string) : undefined,
      to: to != null ? (format(to, 'yyyy-MM-dd') as string) : undefined,
      nights: from && to ? (differenceInDays(to, from) as number) : 0,
    } as {
      from: string;
      to: string;
      nights: number;
